...
void nuisance_constraint()
{
	...
	// Add a constraint to sigma
	RooRealVar sigma_mean("sigma_mean", "Constraint mean", 1.5);
	RooRealVar sigma_err("sigma_err", "Constraint sigma", 0.5);
	RooGaussian sigmaConstraint("sigmaConstraint", "Constraint on sigma", sigma, sigma_mean, sigma_err);

	RooProdPdf constrainedModel("constrainedModel", "Model with sigma Gaussian constraint",
                            RooArgList(Model, sigmaConstraint));
